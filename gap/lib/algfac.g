# -*- Mode: shell-script -*- 
CVSID("$Id: algfac.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
local p,a,K,b,r,x,y,m,c,i,t,pot,s,g,z,o;
  p:=arg[1];
  K:=p.domain.baseRing;
  b:=K.field;
  a:=K.primitiveElm;
  ...
  repeat
    ...
    c:=ShiftedCoeffs(c.coefficients,c.valuation);

    t:=z*x^0*y; # the right 0
    pot:=o*x^0; # the right 1
  for i in c do
    if i in b then
        t:=t+pot*i*x^0;
    else
	t:=t+pot*ValuePol(List(i.coefficients,i->i*x^0),y);
    fi;
      pot:=pot*x;
  od;
...
local a,K,x,y,pp,s,t,i;
  InfoAlg1("#I  factoring using Trager's method\n");
  K:=p.domain.baseRing;
  a:=K.primitiveElm;
  y:=TragerNorm(p,2);
  t:=y[1];
