# -*- Mode: shell-script -*- 
CVSID("$Id: ctbasic.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
#T change function 'ClassNames' ...
    ...
    while col < ncols do
       ...
       od;
       if acol > 1 and len >= linelen then
          acol:= acol-1;
       fi;
...
RealClassesCharTable := function(tbl)
    local i, p, k, oo, po, cla, nccl, powmap, res, map;
       ...
       if oo <= 2 then
          ...
          for p in po do
             if not IsBound(powmap[p]) then
                map:= Powermap(tbl, p, rec( quick:= true ) );
                if Length( map ) = 1 then
                  map:= map[1];
                  powmap[p]:= map;
                else
                  map:= Parametrized( map );
                  if not IsInt( map[cla] ) then
                    Error( "<p>-th power map not available" );
                  fi;
                fi;
             else
               map:= powmap[p];
