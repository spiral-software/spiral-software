# -*- Mode: shell-script -*- 
CVSID("$Id: ctcharac.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
SummandMolienSeries := function( tbl, psi, chi, i )
    ...
    numer:= chi[i] * e ^ Sum( [ 1 .. n ], j -> j * ev[j] ) * x^0;
...
#V  MolienSeriesOps
##
MolienSeriesOps := OperationsRecord( "MolienSeriesOps" );
...
##  
##  is a record that describes the series
...
##  
##  is the <i>-th coefficient of the Molien series <series>.
##  
...
##
##  'pol':      \\ polynomial summand of the series
##                 (always zero?)
##
##  'operations': \\ MolienSeriesOps
##  
...
############################################################################
##
#F  MolienSeriesOps.Print( <series> )
##
MolienSeriesOps.Print := function( molser )

    local z,            # indeterminate
          denom,        # smallest common denominator for the summands
          denomstring,  # string of 'denom', in factored form
          numer,        # numerator of the common fraction
          rkpairs,      # list of pairs of the form '[ r, k ]'
          summand,      # loop over the summands of 'molser'
    ...
    if not IsBound( molser.display ) then

      # Translate the sum of fractions into a single fraction.
      z:= X( Rationals );
      z.name:= "z";
      denom:= z^0;
      denomstring:= "";
      ...
      for summand in molser.summands do

        rr:= summand.r;
        kk:= summand.k;
        sumnumer:= Polynomial( Rationals, summand.numer ) * denom;
        for pair in rkpairs do
          if kk <> 0 and pair[1] mod rr = 0 then
            min:= Minimum( kk, pair[2] );
            sumnumer:= sumnumer / ( 1 - z^rr )^min;
            kk:= kk - min;
          fi;
        od;
        if kk <> 0 then
          # Blow up the common denominator.
          numer:= numer * ( 1 - z^rr )^kk;
          denom:= denom * ( 1 - z^rr )^kk;
          Add( rkpairs, [ rr, kk ] );
      ...
      od;
      numer:= ( numer + denom * Polynomial( Rationals, molser.pol ) ) /
               molser.size;
      if molser.degree mod 2 = 1 then
        numer:= - numer;
      fi;
    ...
    Print( "( ", molser.display.numer, " ) / ( ",
           molser.display.denomstring, " )" );
end;
