# -*- Mode: shell-script -*- 
CVSID("$Id: dispatch.g 9469 2010-03-25 03:22:20Z yvoronen $");
...
Dimensions := function( struct )
	...
	# YSV: .dims() is the method in Spiral style classes
	if IsBound( struct.dims ) then
	    return struct.dims();
	# YSV: .operations is the original GAP way of grouping methods
        elif not IsBound( struct.operations ) then
          Error( "<struct> must have 'operations' entry" );
        fi;
	# GAP does memoization this way, this is not compatible with rewriting
	# because if some fields of the object are updated .dimensions will remain
	# stale. Thus .dims() is used for Spiral-style objects.
	# NOTE: implementation a robust memoization for .dims()
        struct.dimensions:=
