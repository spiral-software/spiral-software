# -*- Mode: shell-script -*- 
CVSID("$Id: domain.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
Domain := function ( elms )
    ...
    elif ForAll( elms, x -> BagType(x) in [T_DOUBLE, T_INT, T_INTPOS, T_INTNEG, T_RAT] )  then
        return Doubles;
    elif ForAll( elms, x -> BagType(x) in [T_CPLX, T_DOUBLE, T_INT, T_INTPOS, T_INTNEG, T_RAT, T_CYC] )  then
        return Complexes;
    else
        for elm  in elms do
