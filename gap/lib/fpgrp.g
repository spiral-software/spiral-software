# -*- Mode: shell-script -*- 
CVSID("$Id: fpgrp.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
WordsOps         := OperationsRecord( "WordsOps", GroupElementsOps );
Words.operations := WordsOps;
...
##
FpGroupOps := OperationsRecord( "FpGroupOps", GroupOps );
...
FpGroupOps.RationalClasses    := FpGroupOps.Centralizer;
...
#############################################################################
##
#F  FpGroupOps.IsElementaryAbelian( <G> ) . test if <G> is elementary abelian
##
...
#############################################################################
##
#F  FpGroupOps.PermGroup( <G> ) . . . . . . . . . . . . . . . permgroup conv.
##
...
##
FpGroupHomomorphismByImagesOps := OperationsRecord
    ( "FpGroupHomomorphismByImagesOps", GroupHomomorphismByImagesOps );
...
FpGroupHomomorphismByImagesOps.CoKernel := function ( hom )
    ...
    #AH added special treatment of free group case (no relators)
    if not IsBound(hom.source.relators) then
      C:=TrivialSubgroup(hom.image);
    else
...
FpGroupHomomorphismByImagesOps.IsMapping := function ( hom )
  if IsFpGroup(hom.range) then
    return GroupHomomorphismByImagesOps.IsMapping(hom);
  fi;
  return hom.source = hom.preImage
     #AH added special treatment of free group case (no relators)
     and ((not IsBound(hom.source.relators)) or
       ForAll( hom.source.relators,
...
LowIndexSubgroupsFpGroup := function ( arg )
    ...
    if exclude then
        ...
        excludeWords := RelsSortedByStartGen(
            G.generators, excludeWords, table, false );
    fi;
