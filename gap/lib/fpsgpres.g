# -*- Mode: shell-script -*- 
CVSID("$Id: fpsgpres.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
AugmentedCosetTableMtc := function ( G, H, ttype, string )
    ...
    if ttype = 0 then
         Error( Concatenation(
        "the function 'AugmentedCosetTableMtc( G, H, type, string )'\n",
        "is currently not available in case type = 0 due to a kernel bug.\n",
        "Please use the function 'AugmentedCosetTableRrs' instead.\n" ) );
    fi;
    ...
    for j in [1..index] do
        ...
        for i  in [ 1 .. numcols ]  do
            ...
            if gen[j] < 0  then
                ...
                gens[numgens] := AbstractGenerator( genname );
                defs[numgens] := word;
...
PresentationAugmentedCosetTable := function ( arg )
    ...
    local aug, coFacTable, comps, gens, i, invs, lengths, numgens, numrels,
          pointers, printlevel, rel, rels, T, tietze, total, tree,
          treelength, treeNums;
...
AbelianInvariantsSubgroupFpGroupMtc := function ( G, H )
    ...
    # temporarily disabled:
    Error( Concatenation(
        "the function 'AbelianInvariantsSubgroupFpGroupMtc'\n",
        "is currently not available due to a kernel bug. Please use\n",
        "the function 'AbelianInvariantsSubgroupFpGroupRrs' instead.\n" ) );
...
RelatorMatrixAbelianizedSubgroupMtc := function ( arg )
    ...
    # temporarily disabled:
    Error( Concatenation(
        "the function 'RelatorMatrixAbelianizedSubgroupMtc'\n",
        "is currently not available due to a kernel bug. Please use\n",
        "the function 'RelatorMatrixAbelianizedSubgroupRrs' instead.\n" ) );
