# -*- Mode: shell-script -*- 
CVSID("$Id: galois.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
local n,i,sh,fu,ps,pps,ind,keineu,avoid;
  ...
  until Length(cand)=1 or keineu>n or p>n*500;
...
local f,n,i,sh,fu,ps,pps,ind,keineu,ba,bk,j,k,a,anz,
  ...
  until Length(cand)<2 or keineu>fac*n or p>n*500;
...
# this function gets all possible orbit lengths from the stored form
...
##
Galois := function(arg)
local f,n,p,sh,p,ind,cand,noca,alt,d,df,co,dco,res,resf,pat,pps,i,j,k,
      orbs,GetResolvent,norb,act,repro,minpol,ext,ncand,pos,step,lens,gudeg,
      typ,pkt,fun,factors,stabs,stanr,nostanr,degs;
  ...
  if Length(Set(co{cand}))>1 then
    ...
    resf:=FactorsOptions(res,rec(onlydegs:=Union(degs)));
    res.factors:=resf;
    pat:=Collected(List(resf,Degree));

    cand:=Filtered(cand,i->dco[i]=pat);
    if Length(Set(co{cand}))>1 then 
      pat:=List(Collected(List(resf,i->ParityPol(i)*Degree(i))),
  ...
  while Length(cand)>1 and Length(repro)>0 do 
    ...
    if Length(Set(co{cand}))>1 then
      if act>=4 and act<=6 then
        ...
        for i in cand do
          ...
          dco[i]:=UnParOrbits(co[i]);
        od;
	degs:=List(Filtered(co,IsBound),GrabCodedLengths);
	...
	res.factors:=resf;
	    ...
	    Product(List(Filtered(Collected(Factors(Numerator(d))),
                               i->not IsInt(i[2]/2)),i->i[1]))/
	    Product(List(Filtered(Collected(Factors(Denominator(d))),
                               i->not IsInt(i[2]/2)),i->i[1]));
        else
