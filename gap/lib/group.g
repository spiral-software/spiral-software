# -*- Mode: shell-script -*- 
CVSID("$Id: group.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
GroupOps.Intersection := function ( G, H )
    ...
    if IsBound( G.parent )  then
        ...
        elif H <> G.parent or not IsGroup(H) then
            return DomainOps.Intersection( G, H );
        fi;
    else
        if IsBound( H.parent )  then
            if G <> H.parent  or not IsGroup(G) then
                return DomainOps.Intersection( G, H );
            fi;
        elif G <> H or not (IsGroup(G) and IsGroup(H)) then
            return DomainOps.Intersection( G, H );
        fi;
    fi;
...
GroupOps.AbelianInvariants := function ( G )
    local   G,  H,  p,  l,  r,  i,  j,  gns,  inv,  ranks, g,  cmm;
