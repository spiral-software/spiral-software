# -*- Mode: shell-script -*- 
CVSID("$Id: grphomom.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
#############################################################################
##
#F  AutomorphismGroup(<G>) . . group of automorphisms, given as Homomorphisms
##
GroupOps.AutomorphismGroup := function(G)
local a,i,inn,b,elms,gp,au,gge;
  # special treatment for abelian groups
  if IsAbelian(G) then
  ...
  if IsGroup(a) then
    ...
    #if ForAll(G.generators,i->i in elms) then
      #gge:=G.generators;
    #else
    # find a generating system in 'elms'
      #gp:=Subgroup(Parent(G),elms{[1,2]});
      #i:=2;
      #while Size(gp)<Size(G) do
	#i:=i+1;
	#while elms[i] in gp do
	  #i:=i+1;
	#od;
	#gp:=Closure(gp,elms[i]);
      #od;
    #  gge:=gp.generators;
    #fi;

    gp:=List(gge,i->Position(elms,i));
    inn:=List(G.generators,k->PermList(List([1..Length(elms)],i->Position(elms,
       elms[i]^k))));
    b:=Group(inn,());
    au:=[];
    ...
    a:=List(Concatenation(inn,au),
	      i->GroupHomomorphismByImages(G,G,gge,List(gp,j->elms[j^i])));
