...
_pkg_resolve2 := function(pkg, usage)
	...
	if not IsGapVar(inner[1]) then Error(usage); fi;
...
#############################################################################
##
#F  Load( <pkg> ) . . . . . . . . . . .  load package (Marek's semantics)
...
Include := UnevalArgs( function ( pkg )
    ...
    if not IsGapVar(pkg) then Error(usage); fi;
    if not IsBound(Local.__dir__) then 
	Error("You can only Include() files from packages loaded with Load()"); fi;
...
LoadRedirect := UnevalArgs( function ( pkg )
    ...
    if not IsGapVar(pkg) then Error(usage); fi;
    if not IsBound(Local.__dir__) then 
	Error("You can only LoadRedirect() from within packages loaded with Load()"); fi;
    path  := Local.__dir__ :: NameOf(pkg);
