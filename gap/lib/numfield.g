# -*- Mode: shell-script -*- 
CVSID("$Id: numfield.g 1667 2005-05-29 08:03:20Z uyvorone $");
    ...
    if IsNumberField( F ) and IsNumberField( G ) then
      ...
      if IsCyclotomicField( F ) then
        ...
        if IsCyclotomicField( G ) then
          ...
          # intersection of cyclotomic field 'F = CF(N)' with number field
        ...
        # intersection of cyclotomic field 'G = CF(N)' with number field 'F';
...
CF := function ( arg )
    ...
    if Length( arg ) = 1  then
      ...
      if xtension = 1 or xtension=2 then
        return Rationals;
    ...
    if IsInt( xtension )  then
      ...
      # Avoid unorthodox Zumbroich bases,
      # which would lead to problems with `Coefficients'.
      if xtension mod 4 = 2 then
        xtension:= xtension / 2;
      fi;
