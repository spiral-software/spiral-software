# -*- Mode: shell-script -*- 
CVSID("$Id: onecohom.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
#F  Info1Coh4( <arg> )  . . . . . . . . . . . . . . package debug information
##
...
if not IsBound( Info1Coh4 )  then Info1Coh4 := Ignore;  fi;
...
OCAgGroupOps.AddMatrices := function( ocr )
    ...
    ocr.matrices := LinearOperation(
    	    	    	...
    	    	    	rec(base:=base,isDomain:=true), function( x, a )  
        ...
        if not IsBound( ocr.smallGeneratingSet )  then
    	    ocr.cocycleToComplement := function( c )
    	    	...
    	    	for i  in [ 1 .. Length( L ) ]  do
    	    	    L[ i ] := gens[ i ] * L[ i ];
    	    	od;
    	    return Subgroup( Parent(ocr.complement), L );
    	    end;
        else
...
OCAgGroupOps.AddCentralizer := function( ocr, B )
    ocr.centralizer := Subgroup(ocr.module, List(B,ocr.vectorMap));
end;
...
OCAgGroupOps.AddComplement := function( ocr, K )
    ...
    ocr.complement := Subgroup( Parent( ocr.group ), K);
end;
...
OCAgGroupOps.CoprimeComplement := function( ocr )
    ...
    if false  then
        for i  in ocr.relators  do
            ...
            for j  in [ 1 .. Length(i.generators) ]  do
                tmp := tmp * ocr.generators[i.generators[j]]
                       ^ i.powers[j];
            od;
            if not tmp in ocr.module  then
                Error( "old relators or generators corrupted" );
            fi;
        od;
        Info1Coh4( "#I  old relators and generators ok\n" );
    fi;

    # get given generators
    gns := List( ocr.pPrimeSet, x -> ocr.generators[x] );

    # check the p prime set
    if false  then
        i := Closure( ocr.module, Subgroup(Parent(ocr.group),gns) );
        j := Size(ocr.module)*Product(List(gns,RelativeOrderAgWord));
        if Size(i) <> j  then
            Error( "old coprime complement corrupted" );
        else
            Info1Coh4( "#I  old coprime complement ok\n" );
        fi;
    fi;
    ...
    if false  then
        for i  in ocr.relators  do
            tmp := ocr.identity;
            for j  in [ 1 .. Length(i.generators) ]  do
                tmp := tmp * K[i.generators[j]]
                       ^ i.powers[j];
            od;
            if not tmp in ocr.module  then
                Error( "new relators or generators corrupted" );
            fi;
        od;
        Info1Coh4( "#I  new relators and generators ok\n" );
    fi;
