# -*- Mode: shell-script -*- 
CVSID("$Id: permag.g 1667 2005-05-29 08:03:20Z uyvorone $");
...
CompositionSeriesSolvablePermGroup := function( G )
	...
	# factor generating representative
        gen:=First(N.generators,i->not i in N.cspgNormal); 
	N.cspgFactor.naturalHomomorphism := GroupHomomorphismByImages(
                N, N.cspgFactor, N.generators, 
          List(N.generators,i->N.cspgFactor.1^
                First([1..Index(N,N.cspgNormal)],j->gen^j/i in N.cspgNormal)));
	N.cspgFactor.naturalHomomorphism.isMapping := true;
