...
String := function( arg )
    ...
    elif Type( obj ) = T_CHAR then str := [ obj ];
    elif IsList( obj )    then str := StringList  ( obj );
    elif IsRec( obj )     then str := StringRec   ( obj );
    elif IsDouble( obj )  then str := StringDouble("%.17g", obj);
    elif IsComplex( obj )  then str := StringComplex("Cplx(%.17g, %.17g)", obj);
...
# Make new string from flattened arg list
MkString := function( arg )
   local s;
   s := String(Concatenation(List(Flat(arg),String)));
   return When(s = [],"",s);
end;
