...
Obj             Log2Int(Obj integer)
{
	...
	if (TNUM_OBJ(integer) == T_INTNEG || TNUM_OBJ(integer) == T_INTPOS) {
		...
		Error("Log2Int: argument must be integer, (not a %s)",
			(Int)TNAM_OBJ(integer), 0);
		return (Obj)NUM_TO_UINT(0); /* please picky cc */
	}
}
	...
	Obj                 opR)
{
	...
	if (ARE_INTOBJS(opL, opR)) {
		if (INT_INTOBJ(opL) == INT_INTOBJ(opR))  return HdTrue;
		else                                       return HdFalse;
	}
	...
	Obj                 opR)
{
	...
	/* compare a small and a large integer                                 */
		...
		if (TNUM_OBJ(opR) == T_INTPOS)  return HdTrue;
		else                              return HdFalse;
	}
	else if (IS_INTOBJ(opR)) {
		if (TNUM_OBJ(opL) == T_INTPOS)  return HdFalse;
		else                              return HdTrue;
	}
	...
	Obj                 opR)
{
	...
	/* adding one large integer and one small integer                      */
		...
		if ((TNUM_OBJ(opL) == T_INTNEG && 0 <= INT_INTOBJ(opR))
			...
			if (TNUM_OBJ(opL) == T_INTPOS)  Retype(opL, T_INTNEG);
			else                              Retype(opL, T_INTPOS);
			sum = DiffInt(opR, opL);
			if (TNUM_OBJ(opL) == T_INTPOS)  Retype(opL, T_INTNEG);
			else                              Retype(opL, T_INTPOS);
			return sum;
	...
	Obj                 op)
{
	...
	/* invert a large integer                                              */
		...
		if (TNUM_OBJ(op) == T_INTPOS && SIZE_INT(op) == 4
			&& ADDR_INT(op)[3] == 0
			&& ADDR_INT(op)[2] == 0
			&& ADDR_INT(op)[1] == (NUM_TO_UINT(1) << (NR_SMALL_INT_BITS - NR_DIGIT_BITS))
			&& ADDR_INT(op)[0] == 0) {
	...
	Obj                 opR)
{
	...
	/* subtracting one small integer and one large integer                 */
		...
		if ((TNUM_OBJ(opL) == T_INTNEG && 0 <= INT_INTOBJ(opR))
			...
			if (c == 1) {
				if (TNUM_OBJ(dif) == T_INTPOS)  Retype(dif, T_INTNEG);
				else                              Retype(dif, T_INTPOS);
			}
			return dif;
		}
		...
		c = (Int)* l++ - (TypDigit)(i / (NUM_TO_UINT(1) << NR_DIGIT_BITS)) + (c < 0 ? -1 : 0);  *d++ = (TypDigit)c;
		...
		if (((UInt*)d == d2
			...
			if (k <= 2 && TNUM_OBJ(dif) == T_INTPOS
				&& (UInt)(INTBASE * d[1] + d[0]) < (NUM_TO_UINT(1) << NR_SMALL_INT_BITS))
	...
	/* subtracting two large integers                                      */
		...
		if ((TNUM_OBJ(opL) == T_INTPOS && TNUM_OBJ(opR) == T_INTNEG)
			|| (TNUM_OBJ(opL) == T_INTNEG && TNUM_OBJ(opR) == T_INTPOS)) {
			if (TNUM_OBJ(opR) == T_INTPOS)  Retype(opR, T_INTNEG);
			else                              Retype(opR, T_INTPOS);
			dif = SumInt(opL, opR);
			if (TNUM_OBJ(opR) == T_INTPOS)  Retype(opR, T_INTNEG);
			else                              Retype(opR, T_INTPOS);
			return dif;
		...
		/* make the right operand the smaller one                          */
		if (SIZE_INT(opL) < SIZE_INT(opR)
			|| (TNUM_OBJ(opL) == T_INTPOS && LtInt(opL, opR) == HdTrue)
			|| (TNUM_OBJ(opL) == T_INTNEG && LtInt(opR, opL) == HdTrue)) {
	...
	/* compute the remainder of a large integer modulo a large integer     */
		...
		if ((m[-4] == 0 && m[-3] == 0 && m[-2] == 0 && m[-1] == 0)
			...
			if (k <= 2 && TNUM_OBJ(mod) == T_INTPOS
				&& (UInt)(INTBASE * m[1] + m[0]) < (NUM_TO_UINT(1) << NR_SMALL_INT_BITS))
	...
	/* divide a large integer by a small integer                           */
		...
		if (SIZE_INT(quo) == 4 && q[-2] == 0 && q[-1] == 0) {
			if (TNUM_OBJ(quo) == T_INTPOS
				...
				< (NUM_TO_UINT(1) << NR_SMALL_INT_BITS))
		...
		if (rs == 0) {
			...
			if ((l[-4] == 0 && l[-3] == 0 && l[-2] == 0 && l[-1] == 0)
				...
				if (k <= 2 && TNUM_OBJ(opL) == T_INTPOS
					&& (UInt)(INTBASE * l[1] + l[0]) < (NUM_TO_UINT(1) << NR_SMALL_INT_BITS))
