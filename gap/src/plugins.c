...
Obj LoadPlugin(char *libname) {
    ...
    if (handle == 0) {
#ifdef WIN32
        return Error("cannot open plugin %s", (Int)libname, 0);
#else
		return Error("%s", (Int)dlerror(), 0);	
#endif
    }
    
    funcptr = dlsym(handle, funcname);
    if (funcptr == 0) {
        return Error("cannot find function %s in library %s", (Int)funcname, (Int)libname);
    }
    
    init_ret = ((init_func)funcptr)(LookupGlobalName);
    if (init_ret != 0) {
        return Error("unable to initialize plugin %s", (Int)libname, 0);
    }
