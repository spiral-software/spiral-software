...
rSetChildFields := arg -> Checked(IsList(arg), 
    Subst(
        meth(self, n, newChild)
            if not n in [1..numfields] then Error("<n> must be in ", [1..numfields]);
            else 
...
rSetChildFieldsCh := arg -> Checked(IsList(arg), 
    Subst(
        meth(self, n, newChild)
            if not n in [1..chCount+numfields] then Error("<n> must be in ", [1..chCount+numfields]);
...
Local._print := function(arg)
    local a;
    for a in arg do 
        if IsRec(a) and IsBound(a.__bases__) then Print(a.__bases__[1]);
        else Print(a); fi;
    od;
end;

Local._children := function(obj)
    #if IsDelay(obj) and not Type(obj) in [T_VAR, T_VARAUTO, T_STRING] then
    #   return obj{[1..Length(obj)]};
    if IsRec(obj) then
        if not IsBound(obj.rChildren) then return [];
        else return obj.rChildren();
        fi;
    elif BagType(obj) in [T_STRING, T_RANGE]
        then return [];
    elif IsList(obj)  #and not IsDelay(obj) 
        then return obj;
    else return [];
    fi;
end;
...
Local._setChild := function(obj, n, newchild)
    ...
    elif IsList(obj) then #and not IsDelay(obj) then 
        obj[n] := newchild;
    else Error("<obj> must be a record or a list");
    fi;
end;
...
Local._fromChildren := function(obj, newchildren)
    ...
    if IsRec(obj) then
        ...
        return obj;
    else return Error("<obj> must be a record or a list");
    fi;
end;
...
Class(@, rec(
    ...
    __call__ := meth(arg)
        ...
        if Length(arg) < 2 or Length(arg) > 4 then 
            Error("Usage: @(<num>, [<target>], [<cond>])"); fi;

        self := arg[1];
        num := arg[2];
        if Length(arg) >= 3 then target := arg[3]; fi;
        if Length(arg) = 4 then precond := arg[4]; fi;

        if not IsBound(self.table.(num)) then
            res := self.new(num); 
            self.table.(num) := res;
            if IsBound(target) then res := res.target(target); fi;
            if IsBound(precond) then res := res.cond(precond); fi;
            return res;
        else
            res := self.table.(num);
            Unbind(res._target);
   ...
   clear := meth(self)
       ...
       for e in l do
           if IsRec(e) and IsBound(e.val) then e.val := false; fi;
       od;
       if IsBound(self.val) then self.val := false; fi;
   end
));
...
Class(@@, @, rec(
    ...
    match := meth(self, obj, cx)
        local base_match;
        base_match := @.match;
        if not IsBound(self._cxcond) then return base_match(self, obj, cx); 
        else return base_match(self, obj, cx) and self._cxcond(obj, cx);
        fi;
    end,
...
#    Is@(shape), 
    IsRec(shape) and IsBound(shape.is@), # NOTE: YSV, unsafe, I removed this -> and shape.is@,
        shape.match(obj, cx), 
...
PatternMatch := function(obj, shape, cx)
    ...
    if   not IsList(shape) or BagType(shape) in [T_STRING,T_RANGE] then return _match_id(obj, shape, cx);
    elif shape = [] then return false; 
    elif not _match_id(obj, shape[1], cx) then return false;
    else 
        shlen := Length(shape);
        ch := _children(obj);
        numch := Length(ch);
...
_normalmatch := function(lst, lstart, llen, shape, sstart, slen, cx)
    local i, ch;
    if llen <> slen then return false;
    else 
        for i in [0..slen-1] do
            if not PatternMatch(lst[lstart+i], shape[sstart+i], cx) then return false; fi;
        od;
        return true;
    fi;
end;
...
_midmatch := function(lst, shape, cx)
    ...
    shlen := Length(shape);
    llen := Length(lst);
    if llen < shlen then return false;
        ...
        for i in [1 .. llen - shlen + 1] do
            if PatternMatch(lst[i], shape[1], cx) then 
                if shlen = 1 then 
                    ...
                    ....right := i+shlen;
                    return true;
                #elif PatternMatch(lst[i+shlen-1], shape[shlen], cx) then
		else
...
_AlternativesRewrite := function(expr, from, to_func, cx, list)
    ...
    ch := _children(expr);
    if Length(ch) <> 0 then # not a leaf
        cx_enter(cx, expr);
...
_SubstBottomUp := function(expr, rules, cx)
    ...
    if ch <> [] then
        cx_enter(cx, expr);
	#expr := map_children_safe(expr, c -> _SubstBottomUp(c, rules, cx));
        for i in [1..Length(ch)] do
...
_SubstLeaves := function(expr, from, to_func, cx)
    ...
    if Length(ch) <> 0 then # not a leaf
        cx_enter(cx, expr);
	#expr := map_children_safe(expr, c -> _SubstLeaves(c, from, to_func, cx));
        for i in [1..Length(ch)] do
...
_SubstTopDownNR := function(expr, rules, cx)
    ...
    cx.rlimit := 1;
    expr := apply_rules_ni(rules, expr, cx);
    if cx.applied > n_applied then return expr; fi;
...
_Pull := function(expr, shape, to_func, pull_func, cx, do_recurse, top_down)
    ...
    if top_down then
        if PatternMatch(expr, shape, cx) then
            ...
            if not do_recurse then return [ data, expr ]; fi;
        fi;
    fi;
