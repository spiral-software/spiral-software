...
Class(RewriteRule, RewriteRuleBase, rec(
	...
	if IsList(from) <#and not IsDelay(from)#> then
            if Length(from)=0 then Error("Rule must have non-empty left-side"); fi;
            op := from[1];
	else
            op := from;
	fi;
...
Class(AssociativeRewriteRule, RewriteRuleBase, rec(
	...
	return WithBases(self, rec(
		   op := op, from := from, to := to, name := name, operations := PrintOps));
    end,
     ...
     compile := meth(self)
        ...
        for r in self.rulesets do
           ...
           if self.warnConflicts then
               self.checkConflicts(rules, r.rules, self.rulesets);  fi;
        ...
        DoForAll(self.rules, (name, rule) ->
		 ...
		 IsList(rule) and not IsString(rule), 
		     DoForAll(rule, r -> self.compileRule(name, r)),
		 0));
...
_ApplyAllRulesTopDown := function(expr, context, ruleset)
    if (not IsRec(expr) or not IsBound(expr.name)) and (not IsList(expr) or BagType(expr) in [T_STRING, T_RANGE])
        then return expr; fi;
...
_ApplyAllRulesBottomUp := function(expr, context, ruleset)
    if (not IsRec(expr) or not IsBound(expr.name)) and (not IsList(expr) or BagType(expr) in [T_STRING, T_RANGE])
        then return expr; fi;
    if IsBound(ruleset.__avoid__) and ObjId(expr) in ruleset.__avoid__ 
        then return expr; fi;
    # recurse
    cx_enter(context, expr);
...
_apply_strategy_step := function(expr, rset, apply_func, opts)
    if IsFunc(rset) then
        if NumArgs(rset)=2 then expr := rset(expr, opts);
        else expr := rset(expr); fi;
    else expr := apply_func(expr, rset.locked(), opts); 
    fi;
    return expr;
end;
...
TDA := function(s, ruleset, opts)
    local cx;
    cx := empty_cx(); cx.opts := opts; cx.applied := 1;
    while cx.applied > 0 do
        cx.applied := 0; s := _ApplyAllRulesTopDown(s, cx, ruleset); od;
    return s;
end;
...
BUA := function(s, ruleset, opts)
    ...
    cx := empty_cx(); cx.opts := opts; cx.applied := 1;
    while cx.applied > 0 do
        cx.applied := 0; s := _ApplyAllRulesBottomUp(s, cx, ruleset); od;
    return s;
end;
